...
`flexible.alpha.hull` <-
  function(data, singleton_buffer = 10000, max_distance = 10000, partCount1 = 10, partCountn = 5)
  {
    
    # # first remove all species with 2 unique records
    coo_2pts <- data.table::setDT(data)[, if (.N == 2) .SD, by = species]
    coo_2pts <- droplevels(coo_2pts)
    # 
    # # get distances between points
    dist2<-list()
     for (i in levels(as.factor(coo_2pts$species))){
       pts2 <- coo_2pts[which(coo_2pts$species == i),]
       d2 <- distm(pts2[1,2:3], pts2[2,2:3], fun = distHaversine)
       dist2[[i]] <- d2
     }
    # 
    ...
    # #write.csv(unique(coo_2pts[,c(1,4)]),"distances_2pts.csv",row.names=F)
    # 
    # # remove species with points more than 10km away from each other, this is assumed to be bad taxonomy or sparse records
    ...
    # 
     coo_1pts <- data.table::setDT(data)[, if (.N == 1) .SD, by = species]
    ...
    # # the buffer we have chosen here is 10km but this can be changed
     hull_buff<-list()
     for (i in levels(as.factor(coo_1pts$species))){
